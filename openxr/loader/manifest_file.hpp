...
class RuntimeManifestFile : public ManifestFile {
   ...
   private:
    ...
    static void CreateIfValid(const std::string &manifest_data, std::vector<std::unique_ptr<RuntimeManifestFile>> &manifest_files);
    static void CreateIfValid(const Json::Value &root_node, const std::string &manifest_data,
