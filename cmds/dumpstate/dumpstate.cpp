...
static const std::string ANR_TRACE_FILE_PREFIX = "trace_";  // Java process traces
...
static void AddAnrTraceDir(const std::string& anr_traces_dir) {
    ...
    // Add Java process traces (these are in the anr directory).
    AddDumps(ds.anr_trace_data_.begin(), ds.anr_trace_data_.end(), "JAVA TRACES",
             true /* add_to_zip */);
}
