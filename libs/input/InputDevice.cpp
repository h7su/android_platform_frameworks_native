...
std::string getInputDeviceConfigurationFilePathByName(
    ...
    std::vector<std::string> rootsForPartition{
            "/product/usr/",
            "/system_ext/usr/",
            "/odm/usr/",
            "/vendor/usr/",
    };
    // These files may also be in the APEX pointed by ro.input.config.apex sysprop.
    if (auto apex = android::base::GetProperty("ro.input.config.apex", ""); apex != "") {
        rootsForPartition.push_back("/apex/" + apex + "/etc/usr/");
    }
    // ANDROID_ROOT may not be set on host
    if (auto android_root = getenv("ANDROID_ROOT"); android_root != nullptr) {
        rootsForPartition.push_back(std::string(android_root) + "/usr/");
    }
