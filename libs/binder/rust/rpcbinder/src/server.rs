...
impl RpcServerRef {
    ...
    /// Sets the per-session root object for this server. Overrides
    /// the root object passed to [`RpcServer::new`].
    ...
    pub fn set_per_session_root_object<F: PerSessionCallback>(&self, f: F) {
        ...
        unsafe {
            binder_rpc_unstable_bindgen::ARpcServer_setPerSessionRootObject(
                ...
                Some(per_session_cb_wrapper::<F>),
                Some(per_session_cb_deleter::<F>),
            );
        }
    }
}

extern "C" fn per_session_cb_wrapper<F: PerSessionCallback>(
    rust_cb: *mut c_char,
    session_ptr: *mut binder_rpc_unstable_bindgen::ARpcSession,
    addr: *const c_void,
    addr_len: size_t,
) -> *mut AIBinder {
    let cb_ptr: *mut F = rust_cb.cast();
...
extern "C" fn per_session_cb_deleter<F: PerSessionCallback>(cb: *mut c_char) {
...
impl tipc::UnbufferedService for RpcServer {
    ...
    fn on_connect(
        ...
        // responsiblity to call ARpcServer_handleTipcChannelCleanup later
        // to clean it up.
        let rc = unsafe {
