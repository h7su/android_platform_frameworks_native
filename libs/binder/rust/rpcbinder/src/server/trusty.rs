...
pub struct RpcServer {
    inner: *mut binder_rpc_server_sys::ARpcServerTrusty,
}
...
impl Drop for RpcServer {
    fn drop(&mut self) {
        ...
        unsafe {
            binder_rpc_server_sys::ARpcServerTrusty_delete(self.inner);
        }
    }
}
...
impl RpcServer {
    ...
    pub fn new(mut service: SpIBinder) -> RpcServer {
        ...
        let inner = unsafe { binder_rpc_server_sys::ARpcServerTrusty_new(service.cast()) };
    ...
    pub fn new_per_session<F: PerSessionCallback>(f: F) -> RpcServer {
        // SAFETY: Takes ownership of the returned handle, which has correct refcount.
        let inner = unsafe {
            binder_rpc_server_sys::ARpcServerTrusty_newPerSession(
...
extern "C" fn per_session_callback_wrapper<F: PerSessionCallback>(
    ...
    len: size_t,
    cb_ptr: *mut c_char,
) -> *mut binder_rpc_server_sys::AIBinder {
...
impl Drop for RpcServerConnection {
    fn drop(&mut self) {
        ...
        unsafe {
            binder_rpc_server_sys::ARpcServerTrusty_handleChannelCleanup(self.ctx);
        }
    }
}
...
impl UnbufferedService for RpcServer {
    ...
    fn on_connect(
        ...
        let rc = unsafe {
            binder_rpc_server_sys::ARpcServerTrusty_handleConnect(
                self.inner,
                handle.as_raw_fd(),
                peer.as_ptr().cast(),
                &mut conn.ctx,
    ...
    fn on_message(
        ...
        let rc = unsafe { binder_rpc_server_sys::ARpcServerTrusty_handleMessage(conn.ctx) };
        if rc < 0 {
            Err(TipcError::from_uapi(rc.into()))
        } else {
    ...
    fn on_disconnect(&self, conn: &Self::Connection) {
        unsafe { binder_rpc_server_sys::ARpcServerTrusty_handleDisconnect(conn.ctx) };
    }
}
