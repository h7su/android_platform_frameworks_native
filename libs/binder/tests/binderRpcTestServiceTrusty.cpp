...
int main(void) {
    ...
    const auto port_acl = RpcServer::TrustyPortAcl{
    ...
    for (auto serverVersion : testVersions()) {
        ...
        auto status = serverInfo.server
                              ->setupTrustyServer(hset, serverInfo.port->c_str(),
                                                  std::shared_ptr<const RpcServer::TrustyPortAcl>(
                                                          &port_acl),
                                                  max_msg_size);
        if (status != OK) {
            return EXIT_FAILURE;
