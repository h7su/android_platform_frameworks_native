...
void writeRandomBinder(borrowed_fd fd, vector<uint8_t>& fillParcelBuffer,
                       stack<ProviderMetadata>& remBytes) {
    ...
    std::string interfaceName(100, 'i');
    // String16 iface(interfaceName.c_str(), interfaceName.size());
    CHECK(WriteFully(fd, interfaceName.c_str(), interfaceName.size())) << fd.get();
...
void writeParcelData(borrowed_fd fd, vector<uint8_t>& fillParcelBuffer,
                     stack<ProviderMetadata>& remBytes, const uint8_t* data, size_t start,
                     size_t length) {
    std::cout << "Writing Parcel Data from " << start << " to " << start + length << " of len "
              << length << std::endl;
...
size_t regenerateParcel(borrowed_fd fd, vector<uint8_t>& fillParcelBuffer, const Parcel& p,
                        size_t dataSize, const vector<size_t>& objectOffsets) {
    stack<ProviderMetadata> remBytesPositions;
    ...
    for (auto offset : objectOffsets) {
        std::cout << "OBJECT OFFSET: " << offset << std::endl;
        ...
        writeParcelData(fd, fillParcelBuffer, remBytesPositions, parcelData, copiedDataPosition,
        ...
        if (flatObject->hdr.type == BINDER_TYPE_BINDER ||
            ...
            ALOGI("Found Binder");
            writeRandomBinder(fd, fillParcelBuffer, remBytesPositions);
            numBinders++;
    ...
    size_t totalWrittenBytes = dataSize - (sizeof(flat_binder_object) * objectOffsets.size()) -
            ...
            (100 /*Interface String*/ + sizeof(bool) + sizeof(status_t)) * numBinders;
    ...
    while (!remBytesPositions.empty()) {
        auto meta = remBytesPositions.top();
        remBytesPositions.pop();
