...
static inline void writeString(binder::borrowed_fd fd, std::string_view str) {
    ...
    LOG_ALWAYS_FATAL_IF(!android::base::WriteFully(borrowedToBase(fd), &length, sizeof(length)));
    LOG_ALWAYS_FATAL_IF(!android::base::WriteFully(borrowedToBase(fd), str.data(), str.length()));
}

static inline std::string readString(binder::borrowed_fd fd) {
    uint64_t length;
    LOG_ALWAYS_FATAL_IF(!android::base::ReadFully(borrowedToBase(fd), &length, sizeof(length)));
    std::string ret(length, '\0');
    LOG_ALWAYS_FATAL_IF(!android::base::ReadFully(borrowedToBase(fd), ret.data(), length));
    return ret;
}
...
static inline binder::unique_fd mockFileDescriptor(std::string contents) {
    ...
    RpcMaybeThread([writeFd = std::move(writeFd), contents = std::move(contents)]() {
        ...
        if (!android::base::WriteStringToFd(contents, borrowedToBase(writeFd))) {
