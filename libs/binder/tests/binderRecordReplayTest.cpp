...
std::vector<uint8_t> retrieveData(binder::borrowed_fd fd) {
    struct stat fdStat;
...
void replayFuzzService(const sp<BpBinder>& binder, const RecordedTransaction& transaction) {
    binder::unique_fd seedFd(open("/data/local/tmp/replayFuzzService",
...
#pragma clang diagnostic pop
    ...
    void recordReplay(Status (IBinderRecordReplayTest::*set)(T), U recordedValue,
        ...
        for (auto replayFunc : replayFunctions) {
            binder::unique_fd fd(open("/data/local/tmp/binderRecordReplayTest.rec",
