...
#pragma clang diagnostic pop
    ...
    template <typename T, typename U>
    void replayFd(Status (IBinderRecordReplayTest::*set)(T), U recordedValue,
                  Status (IBinderRecordReplayTest::*get)(U*), U changedValue) {
        unique_fd fd(
                open("/data/local/tmp/binderReplayFdTest.rec", O_RDWR | O_CREAT | O_CLOEXEC, 0666));
        ASSERT_TRUE(fd.ok());

        // record a transaction
        mBpBinder->startRecordingBinder(fd);
        auto status = (*mInterface.*set)(std::move(unique_fd(dup(recordedValue))));
        EXPECT_TRUE(status.isOk());
        mBpBinder->stopRecordingBinder();
        ...
        // write over the existing state
        status = (*mInterface.*set)(std::move(unique_fd(dup(changedValue))));
        EXPECT_TRUE(status.isOk());
        ...
        const RecordedTransaction& recordedTransaction = *transaction;
        // call replay function with recorded transaction
        replayFuzzService(mBpBinder, recordedTransaction);

        status = (*mInterface.*get)(&output);
        EXPECT_TRUE(status.isOk());

        std::vector<uint8_t> seedData = retrieveData(output);
        // We will be reading from /dev/null
        EXPECT_TRUE(seedData.size() == 0);
    }

    template <typename T, typename U>
    void replayRandomBinder(Status (IBinderRecordReplayTest::*set)(T), U recordedValue,
                            Status (IBinderRecordReplayTest::*get)(U*), U changedValue) {
        unique_fd fd(open("/data/local/tmp/binderReplayBinderTest.rec",
                          O_RDWR | O_CREAT | O_CLOEXEC, 0666));
        ASSERT_TRUE(fd.ok());
        ...
        U rOutput;
        status = (*mInterface.*get)(&rOutput);
        EXPECT_TRUE(status.isOk());
        // EXPECT_EQ(output, recordedValue);
        LOG(ERROR) << "*************INTERFACE: " << rOutput->getInterfaceDescriptor() << " size"
                   << rOutput->getInterfaceDescriptor().size();
        // ALOGE("Iface name %S, size: %zu", rOutput->getInterfaceDescriptor().c_str(),
        // rOutput->getInterfaceDescriptor().size());
    }
...
//     recordReplay(&IBinderRecordReplayTest::setByte, int8_t{122},
//     &IBinderRecordReplayTest::getByte,
//                  int8_t{90});
// }
//
// TEST_F(BinderRecordReplayTest, ReplayBoolean) {
//     recordReplay(&IBinderRecordReplayTest::setBoolean, true,
//     &IBinderRecordReplayTest::getBoolean,
//                  false);
// }
//
...
//     recordReplay(&IBinderRecordReplayTest::setString, input1,
//     &IBinderRecordReplayTest::getString,
//                  input2);
// }
//
...
//     std::vector<char16_t> savedArray = {char16_t{'G'}, char16_t{'L'}, char16_t{'K'},
//     char16_t{'T'}}; std::vector<char16_t> changedArray = {char16_t{'X'}, char16_t{'Y'},
//     char16_t{'Z'}}; recordReplay(&IBinderRecordReplayTest::setCharArray, savedArray,
...
//     std::vector<int64_t> savedArray = {int64_t{1LL << 11}, int64_t{1LL << 55}, int64_t{1LL <<
//     45}}; std::vector<int64_t> changedArray = {int64_t{1LL << 1}, int64_t{1LL << 21}, int64_t{1LL
//     << 33},
//                                          int64_t{1LL << 62}};
...
//                                         String16("\0\0", 2),
//                                         String16("\xF3\x01\xAC\xAD\x21\xAF")};
//
...
TEST_F(BinderRecordReplayTest, ReplayBinder) {
    ...
    sp<IBinder> saved = new RandomBinder(String16("tssdsdsd"), std::move(data));
    sp<IBinder> changed = IInterface::asBinder(defaultServiceManager());
    replayRandomBinder(&IBinderRecordReplayTest::setBinder, saved,
                       &IBinderRecordReplayTest::getBinder, changed);
}
...
//      changed.get();
//
//      // When fds are replayed, it will be replaced by /dev/null..reading from it should yeild
//      null data replayFd(&IBinderRecordReplayTest::setFileDescriptor,
//      std::move(unique_fd(dup(saved))), &IBinderRecordReplayTest::getFileDescriptor,
//      std::move(unique_fd(dup(changed))));
// }
