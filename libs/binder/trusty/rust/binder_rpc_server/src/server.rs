...
impl RpcServer {
    ...
    pub fn new(mut service: SpIBinder) -> RpcServer {
        ...
        let inner = unsafe { CRpcServer::from_ptr(RpcServerTrustyRust_new(service.cast())) };
    ...
    pub fn new_per_session(f: impl PerSessionCallback) -> RpcServer {
        ...
        // SAFETY: Takes ownership of the returned handle, which has correct refcount.
        let inner = unsafe {
            CRpcServer::from_ptr(RpcServerTrustyRust_newPerSession(
...
impl UnbufferedService for RpcServer {
    ...
    fn on_message(
        ...
        let rc = unsafe { RpcServerTrustyRust_handleMessage(conn.ctx) };
        if rc < 0 {
            Err(TipcError::from_uapi(rc.into()))
        } else {
    ...
    fn on_disconnect(&self, conn: &Self::Connection) {
        unsafe { RpcServerTrustyRust_handleDisconnect(conn.ctx) };
    }
}
