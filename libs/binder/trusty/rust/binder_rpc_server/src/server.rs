...
foreign_type! {
    type CType = binder_rpc_unstable_bindgen::ARpcServerTrusty;
    fn drop = binder_rpc_unstable_bindgen::ARpcServerTrusty_delete;

    /// A type that represents a foreign instance of RpcServer.
    #[derive(Debug)]
    pub struct CRpcServer;
    /// A borrowed RpcServer.
    pub struct CRpcServerRef;
}

/// SAFETY: The opaque handle can be cloned freely.
unsafe impl Send for CRpcServer {}
/// SAFETY: The underlying C++ RpcServer class is thread-safe.
unsafe impl Sync for CRpcServer {}
...
impl RpcServer {
    ...
    pub fn new(mut service: SpIBinder) -> RpcServer {
        ...
        let inner = unsafe {
            CRpcServer::from_ptr(binder_rpc_unstable_bindgen::ARpcServerTrusty_new(service.cast()))
        };
    ...
    pub fn new_per_session(f: impl PerSessionCallback) -> RpcServer {
        ...
        // SAFETY: Takes ownership of the returned handle, which has correct refcount.
        let inner = unsafe {
            CRpcServer::from_ptr(binder_rpc_unstable_bindgen::ARpcServerTrusty_newPerSession(
...
extern "C" fn per_session_cb_wrapper(
    ...
    // SAFETY: This callback should only get called while the CRpcServer is alive.
